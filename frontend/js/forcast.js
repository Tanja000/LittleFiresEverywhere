...
function updateCircleRadius(map, initialRadius, redCircle) {
    const zoomLevel = map.getZoom();
    const adjustedRadius = initialRadius / Math.pow(2, zoomLevel - 10); // Adjust as needed
    redCircle.setRadius(adjustedRadius);
}
...
function addRedCircleToIcon(latitude, longitude, map, circles){
    ...
    if(circles.length === 0){
        ...
        map.on('zoomend', updateCircleRadius(map, initialRadius, redCircle));
        return circles;
    }
    else {
        ...
        /*
           if (step > points.length) {
            ...
            if (step < points.length) {
                ...
                let slowDownFactor = meteoData['soil_temperature_0cm'] + meteoData['relativehumidity'];
                let speedFactor = meteoData['evapotranspiration'];
                let rain = meteoData['rain'];
                let temp_factor = meteoData['soil_temperature_0cm'] / 40 + meteoData['temperature_2m'] / 40 ;
                let factor = (1 + speedFactor) / slowDownFactor;

                let radius = factor * temp_factor + initialRadius;
                if (rain > 0){
                ...
                const hullPolygon = new L.polygon(convexHull.geometry.coordinates, { color: '#8b0000', fillOpacity: 0.5, name: 'forcast-polygon' }).addTo(map);
...
export function getForcastLayer(responseData, map, circles, meteoData) {
    ...
    for (const key in responseData) {
        ...
        for (const key in response) {
            ...
            const firstCoordinate = Object.values(first['0']);
            circles = addRedCircleToIcon(firstCoordinate[0], firstCoordinate[1], map, circles);
        }
